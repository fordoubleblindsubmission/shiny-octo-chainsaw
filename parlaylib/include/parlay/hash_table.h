...
class hashtable {
 private:
  ...
  index hashToRange(index h) const { return static_cast<index>(static_cast<size_t>(h) % m); }
  index firstIndex(kType v) const { return hashToRange(hashStruct.hash(v)); }
  index incrementIndex(index h) const { return (h + 1 == m) ? 0 : h + 1; }
...
struct hash_numeric {
  ...
  kType getKey(eType v) const { return v; }
  size_t hash(kType v) const { return static_cast<size_t>(hash64(v)); }
  int cmp(kType v, kType b) const { return (v > b) ? 1 : ((v == b) ? 0 : -1); }
...
struct hash_pair {
  ...
  using eType = struct pair {
    T first;
    U second;
    bool operator==(const pair &rhs) const { return this->first == rhs.first && this->second == rhs.second; };
  };
  using kType = T;
  eType empty() { return {0, 0}; }
  kType getKey(eType v) const { return v.first; }
  size_t hash(kType v) const { return static_cast<size_t>(hash64(v)); }
  int cmp(kType v, kType b) const { return (v > b) ? 1 : ((v == b) ? 0 : -1); }
  bool replaceQ(eType, eType) { return true; }
  eType update(eType v, eType b) { return {v.first, b.second}; }
  bool cas(eType *p, eType o, eType n) {
    // TODO: Make this use atomics properly. This is a quick
    // fix to get around the fact that the hashtable does
    // not use atomics. This will break for types that
